...
#if !DEBUG_MODE
	std::cout << "Cartridge loaded " << std::dec << (static_cast<int>((_data.size() / 1024.0) * 10) / 10.0) << " kB"
			  << std::endl;
#endif
...
bool Cartridge::retrieveHeaderContent(int iAddress)
{
	...
	if(static_cast<bool>(memcmp(headerBuffer, "TMR SEGA", 8) == 0)) {
		...
		_header.productCode = (_data[iAddress + size_t(12)] << 8) | _data[iAddress + size_t(13)]
			| (_data[iAddress + size_t(14)] & 0xF0) << 12;
