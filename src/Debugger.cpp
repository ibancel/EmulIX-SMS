...
Debugger::State Debugger::manage(uint_fast64_t iCurrentAddr)
{
	...
	if(_actualState == State::kRunning) {
		for(std::vector<std::unique_ptr<Breakpoint>>::iterator it = _breakpointList.begin();
			...
			bool toErase = false;
			if(breakpoint->getType() == Breakpoint::Type::kNumInstruction
				&& breakpoint->getValue() == _instructionCounter) {
				pause();
...
void Debugger::pause() { _actualState = Debugger::State::kPaused; }

void Debugger::resume() { _actualState = Debugger::State::kRunning; }

void Debugger::step() { _requestStep = true; }

void Debugger::addNumberCycle(uint_fast64_t iTstatesCycle) { _cycleCounter += iTstatesCycle; }

Debugger::State Debugger::getState() const { return _actualState; }
