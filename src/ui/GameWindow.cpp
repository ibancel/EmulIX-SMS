...
void GameWindow::drawFrame(const Frame& iFrame) { _frameToDraw = iFrame; }
...
QSize GameWindow::sizeHint() const { return QSize(_resolutionWidth, _resolutionHeight); }
...
{
	_textureFrame->setData(
		QOpenGLTexture::RGBA, QOpenGLTexture::UInt8, _frameToDraw.data(FrameOriginFormat::kBottomLeft));
...
void GameWindow::resizeGL(int iWidth, int iHeight) { updateProjection(); }
...
void GameWindow::updateProjection()
{
	...
	_projection.ortho(
		0.0f, static_cast<float>(_resolutionWidth), static_cast<float>(_resolutionHeight), 0.0f, 0.0f, 1.0f);
