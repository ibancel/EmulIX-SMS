...
// Only for x == 1:
const std::string OPCODE_ED_NAME[] = { "IN r[y], (C)", "IN r[y], (C)", "IN r[y], (C)", "IN r[y], (C)", "IN r[y], (C)",
	"IN r[y], (C)", "IN (C)", "IN r[y], (C)", "OUT (C), r[y]", "OUT (C), r[y]", "OUT (C), r[y]", "OUT (C), r[y]",
	"OUT (C), r[y]", "OUT (C), r[y]", "OUT (C), 0", "OUT (C), r[y]", "SBC HL, rp[p]", "ADC HL, rp[p]", "-", "-", "-",
	"-", "-", "-", "LD (nn), rp[p]", "LD rp[p], (nn)", "-", "-", "-", "-", "-", "-", "NEG", "NEG", "NEG", "NEG", "NEG",
	"NEG", "NEG", "NEG", "RETN", "RETI", "RETN", "RETN", "RETN", "RETN", "RETN", "RETN", "IM im[y]", "IM im[y]",
	"IM im[y]", "IM im[y]", "IM im[y]", "IM im[y]", "IM im[y]", "IM im[y]", "LD I, A", "LD R,A", "LD A, I", "LD A, R",
	"RRD", "RLD", "NOP", "NOP" };

std::string getOpcodeName(u8 prefix, u8 opcode);

// pos from 0 to 7 (with 7 MSB)
inline u8 getBit8(u8 value, u8 pos) { return (value >> pos) & 0b1; }
...
inline u8 getLowerByte(u16 value) { return (value & 0xFF); }
...
inline u8 sign8(u8 byte) { return byte >> 7; }
inline u16 sign16(u16 word) { return word >> 15; }

inline bool isPositive8(u8 byte) { return static_cast<bool>(sign8(byte)); }
inline bool isPositive16(u16 word) { return static_cast<bool>(sign16(word)); }
